...
Run node bamazonManager.js on the command line */
...
function addInventory(){
	console.log("Take a look! Products are listed as ID# -- ITEM NAME -- QUANTITY(#)");
	...
	connection.query("SELECT * FROM products", function(itemsErr, itemsRes){
		...
		for (var k = 0; k < itemsRes.length; k++){
			var itemsDisplay = "ID# " + itemsRes[k].item_id + "-- ITEM NAME " + itemsRes[k].product_name + " -- QUANTITY(#): " + itemsRes[k].stock_quantity;
...
function addProduct(){
	inquirer.prompt([{
		...
		validate: function(quantity){
			...
			console.log("Please enter a valid quantity number to be added!");
			return false;
		}
